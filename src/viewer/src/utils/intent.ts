...
export function analyzeIntentAlignment(
  ...
  if (uncoveredChanges.length === 0 && coveredChanges.length > 0) {
    ...
    // If there are breaking changes, show PARTIAL regardless of coverage
    // Intent exists but may not perfectly match detected changes
    status = "PARTIAL";
  } else {
    // No breaking changes detected - intent is aligned by default
    status = "ALIGNED";
  }
